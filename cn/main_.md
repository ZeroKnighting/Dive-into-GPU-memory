...
&emsp;&emsp;sentenceLength对显存的影响主要体现在对中间激活变量activations的影响，有的是O(n)的关系，而有的是<img src="https://latex.codecogs.com/gif.latex?O(n^2)"/>的关系，主要取决于选取算子的影响。不同算子对计算会有不同需要保存的数据，这部分我们在后文中进行详细分析。而对于bert模型来说，其输入文本长度与显存占用是<img src="https://latex.codecogs.com/gif.latex?O(n^2)"/>的关系，这也是为什么bert很难适应于长文本的原因
...
&emsp;&emsp;我们再对其进行拟合，可以得到拟合曲线`0.00123 x^2 + 0.6039 x + 437.9`符合预期
...
&emsp;&emsp;Adafactor，该优化器对于每一个参数只需要额外的4bytes，如果模型参数量为2GB，则额外的显存占用为2\*4=8GB，该优化器不同于保留每个权重矩阵元素的滚动平均值，它只存储行和列的滚动平均值总和，从而大大减少了内存占用。Adafactor的一个缺点是，在某些情况下，收敛速度可能比Adam慢
&emsp;&emsp;8-bit Adam，该优化器对于每一个参数只需要额外的2bytes，如果模型的参数量为2GB，则额外的显存占用为2\*2=4GB。该优化器保存完整的状态数据，但是会对其进行量化，使得数据能够以更小的精度存储起来，从而使得显存占用降低，当需要使用的时候进行反量化。
  
&emsp;&emsp;我们使用刚才的数据集，对使用Adafactor的bert做实验（之前实验使用的优化器为Adam），训练程序占用的显存为9131MB，时间为179.71s，Samples/second为2.85，相较于默认的训练方法，该方法最终减少了2408MB的显存占用。但是正如刚才所说，该优化器可能效果不如Adam，在我们训练时根据需要进行选择
&emsp;&emsp;如果使用8-bit Adam的bert进行实验，则最终程序的显存占用为9529 MB，时间为177.50s，Samples/second为2.88，相较于默认的训练方法，该方法最终减少了2010MB的显存占用。可以看到，我们在保持梯度的完整滚动平均值的同时，获得了与Adafactor相似的内存改进。
  
<center>
    <img width="40%"
    style="border-radius: 0.3125em;
    box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" 
    src="./pic/op_compare.png">
    <br>
    <div style="color:orange; border-bottom: 1px solid #d9d9d9;
    display: inline-block;
    color: #999;
    padding: 2px;">comparison</div>
</center>
